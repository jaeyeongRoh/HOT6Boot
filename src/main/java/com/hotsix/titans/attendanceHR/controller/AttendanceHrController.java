...
public class AttendanceHrController {
    ...
    /* 모달창 저장 누를 시 */
    @Operation(summary = "모달창 수정값 저장", description = "근태 상태를 바꾼후 저장을 누르면 DB 저장", tags = { "AttendanceHrController" })
    ...
    @Operation(summary = "나의 근태 이력 조회 요청", description = "나의 근태 이력을 리스트로 조회", tags = { "AttendanceHrController" })
    ...
    @Operation(summary = "사유서 업로드", description = "근태 정정 사유서 파일 업로드", tags = { "AttendanceHrController" })
    ...
    @Operation(summary = "사유서 다운로드", description = "근태 정정 사유서 파일 다운로드", tags = { "AttendanceHrController" })
    ...
    public ResponseEntity<ResponseDTO> downloadFile(@PathVariable String commuteNo) {
        ...
        return ResponseEntity.ok().body(new ResponseDTO(HttpStatus.OK, "파일 접근 성공", attendanceHrService.selectFile(commuteNo).getAttendanceHrReasonList()));
    }
